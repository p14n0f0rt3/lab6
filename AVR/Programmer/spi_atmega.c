...
//
// Atmega328/168 (dip28) pin:    BO230XS board:
//      (GND) 8  --- GND
//      (VCC) 7  --- VCC
//      (GND) 22 --- GND
//      (PB3) 17 --- TXD
...
//  (c) Jesus Calvino-Fraga (2016-2022)
//
...
//  On Linux using gcc:
//
//  $ gcc spi_atmega.c -lpthread libftd2xx.a -o spi_atmega
//
...
//
//  $ sudo ./spi_atmega -CRYSTAL -p -v blinky.hex
//
...
void Fuses_Atmega328 (void)
{
	...
	CONFIGFUSE(BODLEVEL2,  Fuse_Extended_Bits, BIT2);
	CONFIGFUSE(BODLEVEL1,  Fuse_Extended_Bits, BIT1);
	CONFIGFUSE(BODLEVEL0,  Fuse_Extended_Bits, BIT0);
}

void Fuses_Atmega168 (void)
{
	// Defaults
	Fuse_Extended_Bits=0xfa;
	Fuse_High_Bits=0xdf;
	Fuse_Low_Bits=0x62;
	...
	CONFIGFUSE(BOOTSZ1, Fuse_Extended_Bits, BIT2);
	CONFIGFUSE(BOOTSZ0, Fuse_Extended_Bits, BIT1);
	CONFIGFUSE(BOOTRST, Fuse_Extended_Bits, BIT0);
}
...
int Identify_Atmega (void)
{
    ...
    else if(Signature_Byte[0]==0x1e && Signature_Byte[1]==0x94 && Signature_Byte[2]==0x06)
    {
		printf("Atmega168A detected.\n"); fflush(stdout);
		memsize=0x4000;
		eesize=512;
		Fuses_Atmega168();
    }
    else if(Signature_Byte[0]==0x1e && Signature_Byte[1]==0x94 && Signature_Byte[2]==0x0b)
    {
		printf("Atmega168AP detected.\n"); fflush(stdout);
		memsize=0x4000;
		eesize=512;
		Fuses_Atmega168();
    }
...
int main(int argc, char ** argv)
{
    ...
    printf("Atmega programmer using BO230X board. (C) Jesus Calvino-Fraga (2016-2022)\n");
